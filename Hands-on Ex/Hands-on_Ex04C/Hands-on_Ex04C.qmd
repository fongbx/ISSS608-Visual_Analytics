...
exam_data %>% 
  ...
  stat_pointinterval(.width = 0.95,
                     .point = median,
                     .interval = qi) +
  labs(title = "Visualising confidence intervals of median maths score",
       subtitle = "Median point + Multiple-interval plot")
```

Next, we will makeover the plot by showing the difference levels of confidence intervals. We can do so by using [`stat_gradientinterval()`](#0){style="font-size: 11pt;"} of **ggdist** is used to build a visual for displaying distribution of maths scores by race.

```{r}
exam_data %>% ggplot(aes(x = RACE, y = MATHS)) +
...
For this we will need to use the ungeviz package. Download the package using `devtools::install_github("wilkelab/ungeviz")` and load using `library(ungeviz)`.

We can generate the HOP using the code chunk below.

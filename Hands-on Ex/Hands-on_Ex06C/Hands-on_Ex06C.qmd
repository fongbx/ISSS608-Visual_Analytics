...
We will use `read_csv()` of readr to import the dataset into R and parse it into tibble R dataframe format.

```{r}
wh <- read_csv("data/WHData-2018.csv")
```

### Change Country to rowname
...
The data was loaded into a data frame, but it has to be a data matrix to make a heatmap. The code chunk below will be used to transform *wh1* data frame into a data matrix.

```{r}
wh1 <- dplyr::select(wh, c(2, 6:11))
...
In this section, we will plot a heatmap using *heatmap()* of Base Stats. The code chunk is given below.
...
To plot a cluster heatmap, we just use the default as shown in the code chunk below.
...
Here, red cells denotes high values, and lighter colour low ones. This heatmap is not really informative. Indeed, the Happiness Score variable have relatively higher values, which makes that the other variables with low values all look the same. Thus, we need to normalise this matrix. This is done using the *scale* argument. It can be applied to rows or to columns following your needs.
...
## Basic heatmaply plot

The code chunk below shows the basic syntax needed to create an interactive heatmap using **heatmaply** package.
...
-   This is similar to ranking the variables, but instead of keeping the rank values, it divides them by the maximal rank.
...
heatmaply(normalize(wh_matrix),
          ...
          fontsize_row = 4,
          fontsize_col = 5,
          main = "Main Happiness Score and Variables by Country",
...
-   [Heatmap for Visualising and Analysing Mutivariate Data](https://r4va.netlify.app/chap14.html#clustering-algorithm)
